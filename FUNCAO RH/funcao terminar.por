	...
	funcao real calculo_ferias(real salario, real venda_ferias)
{
	...
	funcao fgtss()
	{
		real salario, horas_extras = 0.0, periculosidade = 0.0, adicional_noturno = 0.0, valor_total = 0.0, hora_extras_feitas_no_mes = 0.0
		cadeia pericul, adicional_n, hora_ex
		cadeia funcao_de_trabalho = "E"
		escreva("informe seu salario:")
		leia(salario)
      funcao calculo_fgts()
          escreva("possui horas extras?")
          leia(hora_ex)
          hora_ex = TXT.caixa_alta(hora_ex)
          se(hora_ex =="S")
		horas_extras = calculo_hora_extra(salario, horas_extras)
		
          escreva("possui periculosidade?")
          leia(pericul)
          pericul = TXT.caixa_alta(pericul)
          se(pericul == "S")
          periculosidade = calculo_periculosidade( funcao_de_trabalho, salario)
          
          	

		escreva("possui adicional noturno?")
		leia(adicional_n)
		adicional_n = TXT.caixa_alta(adicional_n)
		se(adicional_n == "S")
		
		adicional_noturno = calculo_adicional_noturno( salario, hora_extras_feitas_no_mes)
		adicional_noturno = adicional_noturno - salario

		valor_total = horas_extras + periculosidade + adicional_noturno
		valor_total = (valor_total / 100) * 8
		escreva("seu fgts depositado mensalmente e de:R$",valor_total )
